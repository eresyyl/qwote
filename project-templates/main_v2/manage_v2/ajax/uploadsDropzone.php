...
if (isset($_FILES) ) {
	...
	if ( $movefile && !isset( $movefile['error'] ) ) {
		...
		require_once( ABSPATH . 'wp-admin/includes/image.php' );
                ...
                if (isset($_SESSION['projectUploads'])) {
                    $projectUploads = $_SESSION['projectUploads'];
                }
                else {
                    $projectUploads = array();
                }
                // add current uploadId to cookie array
                $projectUploads[] = $attach_id;
                // saving current upload to cookie
                //setcookie('projectUploads', serialize($projectUploads), time()+3600);
                $_SESSION['projectUploads'] = $projectUploads;

                var_dump($projectUploads);
