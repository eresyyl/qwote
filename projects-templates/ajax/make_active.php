<?php 
/*
This is AJAX function for Head or Agent to make Quote (Project) Active.
Client will get a notification about changing status: Quote -> Active.
...
if($_POST && $_POST['quote_id']) {
        ...
        update_field('field_567db3a488cb0','active',$quote_id);
        
        // make Notification for Client
        $notification_text = $who . ' updated your job details.';
        go_notification($notification_text,'waiting',$date,$client_id,$quote_id);
        
        // make Activity row
        $activity_text = $who . ' updated your job. Waiting for client review.';
        ...
        */
        ...
        $text = '<p>' . $who . ' updated details of your job..</p><p>You can check it out here: ' . get_bloginfo('url') . '/?p=' . $quote_id . '</p>';
        go_email($subject,$title,$text,$client_id);
        
        // showing response message
        echo "<div class='text-center selection_add_success margin-vertical-20 green-600'>Project is active!</div>";
        if($single == 'true') {
