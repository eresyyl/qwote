...
$status_relation = array('relation'=>'AND');

// get search params
$user_args = array();
if($_GET['title'] || $_GET['client']) {
        $s_title = $_GET['title'];
        ...
        if($s_client) {
                ...
                foreach($all_clients as $client) {
                        ...
                        if (strpos($client_name_to_search, $s_client) !== false) {
                                $user_args[] = array('key'=>'client_id','value'=>$client,'compare'=>'LIKE');
                                $clients_to_show[] = intval($client);
                        }
                }
        }
        var_dump($clients_to_show);
        
}
$clients_relation = array('relation'=>'OR');
$clients_merged = array_merge($clients_relation,$user_args);

if($quote_status != 'all') {
        $status_merged = array_merge($status_relation,$status_args);
        $clients_merged = array($clients_merged);
        $meta_merged = array_merge($status_merged,$clients_merged);
}
else {
        $meta_merged = $clients_merged;
}

$meta_args = array('meta_query' => $meta_merged);
